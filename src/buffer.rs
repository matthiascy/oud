...
/// A contigunous non-growable GPU buffer containing sub-buffer ranges with the same buffer usages.
...
impl SlicedBuffer {
    ...
    /// Create a new buffer with the given capacity, usages and sub-buffer ranges.
    pub fn new_sliced(
        device: &wgpu::Device,
        cap: wgpu::BufferAddress,
        usages: wgpu::BufferUsages,
    ...
    /// Create a new buffer with the initial data, capacity, usages and sub-buffer ranges.
    ...
    pub fn num_sub_buffers(&self) -> usize {
        self.subslices.len()
    }
    ...
    pub fn subslices(&self) -> &Vec<Range<wgpu::BufferAddress>> {
        &self.subslices
    }
    ...
    /// Get the buffer slices.
    pub fn subslices_mut(&mut self) -> &mut Vec<Range<wgpu::BufferAddress>> {
        &mut self.subslices
    }
